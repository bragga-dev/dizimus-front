...
import {
  User, Mail, Phone, MapPin, FileText, Pencil, Camera, Trash2,
  X, Check, Loader2, ChevronDown, Calendar, Shield, AlertCircle,
  Plus, Home
} from 'lucide-react'
...
import {
  ...
  getMemberAddresses, createMemberAddress,
  updateMemberAddress, deleteMemberAddress,
...
function Toast({ message, type = 'success', onClose }) {
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-[200] flex items-center gap-3 px-4 py-3 rounded-2xl shadow-2xl border text-sm font-medium transition-all animate-slide-up ${
      type === 'success'
...
function Modal({ title, onClose, children, size = 'md' }) {
  ...
  return (
    ...
    >
      ...
      {/* Panel */}
      <div className={`relative w-full ${widths[size]} bg-gradient-to-b from-[#1e0035] to-[#0c0620] border border-[#E0B14A]/15 rounded-t-3xl sm:rounded-3xl shadow-[0_25px_80px_rgba(0,0,0,0.6)] max-h-[92vh] overflow-y-auto`}>
        {/* Handle mobile */}
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
          <h2 className="text-base font-semibold text-white" style={{ fontFamily: 'var(--font-navbar)' }}>
            {title}
          </h2>
          <button
            onClick={onClose}
  ...
  return (
    <div>
      <label className="block text-xs font-semibold text-white/50 uppercase tracking-wider mb-1.5">
        {label}{required && <span className="text-[#E0B14A] ml-1">*</span>}
      </label>
      <input
        type={type}
        name={name}
  ...
  return (
    <div>
      ...
      <div className="relative">
        ...
        >
          <option value="" className="bg-[#1e0035]">{placeholder ?? 'Selecione...'}</option>
          {options.map(o => (
            <option key={o.value} value={o.value} className="bg-[#1e0035]">{o.label}</option>
          ))}
        </select>
  ...
  return (
    <div className="flex items-start gap-3 py-3 border-b border-white/5 last:border-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className={`text-sm mt-0.5 ${value ? 'text-white' : 'text-white/25 italic'}`}>
          {value || empty}
        </p>
      </div>
    </div>
...
function SectionCard({ title, icon: Icon, onEdit, editLabel = 'Editar', children, badge }) {
  return (
    <div className="bg-white/3 border border-white/8 rounded-2xl overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-white/5">
        <div className="flex items-center gap-2.5">
          ...
          <h3 className="text-sm font-semibold text-white" style={{ fontFamily: 'var(--font-navbar)' }}>
            {title}
          </h3>
          {badge}
        </div>
    ...
    <Modal title="Foto de perfil" onClose={onClose} size="sm">
      ...
      <div className="flex justify-center mb-6">
        <div className="relative w-28 h-28">
          {photoUrl
            ? <img src={photoUrl} alt="Foto de perfil" className="w-full h-full rounded-2xl object-cover border-2 border-[#E0B14A]/20" />
            : (
    ...
    <Modal title="Dados pessoais" onClose={onClose} size="md">
      <div className="space-y-4">
        ...
        <div className="pt-2 border-t border-white/5">
          <p className="text-xs text-white/25 mb-3 uppercase tracking-wider font-semibold">Em breve</p>
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="flex items-end">
            ...
            >
              {cepLoading ? <Loader2 size={13} className="animate-spin" /> : null}
...
export default function MemberHome() {
  ...
  const handlePhotoUpdate = (updated) => {
    // Atualiza o AuthContext com a nova photo_url
    if (updated?.photo_url && user) {
      // Força o AuthContext a refletir a nova foto
      saveSession && saveSession({ access: localStorage.getItem('access_token'), refresh: localStorage.getItem('refresh_token') })
    }
    setModal(null)
  ...
  const mainAddress = addresses.find(a => a.principal) ?? addresses[0] ?? null

  const addressString = mainAddress
    ? [mainAddress.road, mainAddress.number, mainAddress.district, mainAddress.city, mainAddress.state]
        .filter(Boolean).join(', ')
    : null
          ...
          <div className="relative flex flex-col sm:flex-row items-center sm:items-end gap-5 p-6">
            ...
            <div className="relative group flex-shrink-0">
              <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl overflow-hidden border-2 border-[#E0B14A]/20 shadow-[0_0_30px_rgba(224,177,74,0.1)]">
                {user?.photo_url
                  ? <img src={user.photo_url} alt={fullName || 'Foto de perfil'} className="w-full h-full object-cover" />
                  : (
            ...
            <div className="flex-1 text-center sm:text-left">
              ...
              {profile?.username && (
                <p className="text-sm text-[#E0B14A]/70 mt-0.5">@{profile.username}</p>
              )}
            ...
            <div className="flex items-center gap-2.5">
              ...
              {addresses.length > 0 && (
                <span className="text-xs text-white/30 font-medium">
                  {addresses.length} {addresses.length === 1 ? 'cadastrado' : 'cadastrados'}
                </span>
              )}
            </div>
            <button
              onClick={() => { setEditAddress(null); setModal('address') }}
          ...
          <div className="px-5 py-2">
            {addresses.length === 0 ? (
              ...
              addresses.map((addr, i) => (
                <div key={addr.id ?? i} className="py-3 border-b border-white/5 last:border-0">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      <button
                        onClick={() => { setEditAddress(addr); setModal('address') }}
                      ...
                      <button
                        ...
                        className="h-7 w-7 flex items-center justify-center rounded-lg text-white/30 hover:text-red-400 hover:bg-red-500/8 transition-all"
                        title="Remover"
                      >
                        <Trash2 size={13} />
                      </button>
